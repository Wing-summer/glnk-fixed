    ...
    public partial class FormCollection : Form
    {
        ...
        public FormCollection(Root root)
        {
            ...
            for (int b = 0; b < Root.MaxPenCount; b++)
            {
                ...
                btPen[b].Click += new EventHandler(btColor_Click);
            ...
            LastTickTime = DateTime.Parse("2021-04-27");
            tiSlide.Enabled = true;

            ToTransparent();
            ToTopMost();
            toolTip.SetToolTip(btDock, Root.Local.ButtonNameDock);
            toolTip.SetToolTip(btPenWidth, Root.Local.ButtonNamePenwidth);
            toolTip.SetToolTip(btEraser, Root.Local.ButtonNameErasor + " (" + Root.Hotkey_Eraser.ToString() + ")");
            toolTip.SetToolTip(btPan, Root.Local.ButtonNamePan + " (" + Root.Hotkey_Pan.ToString() + ")");
            toolTip.SetToolTip(btPointer, Root.Local.ButtonNameMousePointer + " (" + Root.Hotkey_Pointer.ToString() + ")");
            toolTip.SetToolTip(btInkVisible, Root.Local.ButtonNameInkVisible + " (" + Root.Hotkey_InkVisible.ToString() + ")");
            toolTip.SetToolTip(btSnap, Root.Local.ButtonNameSnapshot + " (" + Root.Hotkey_Snap.ToString() + ")");
            toolTip.SetToolTip(btUndo, Root.Local.ButtonNameUndo + " (" + Root.Hotkey_Undo.ToString() + ")");
            toolTip.SetToolTip(btClear, Root.Local.ButtonNameClear + " (" + Root.Hotkey_Clear.ToString() + ")");
            toolTip.SetToolTip(btStop, Root.Local.ButtonNameExit + " (ESC)");
        }
        ...
        private void IC_MouseUp(object sender, CancelMouseEventArgs e)
        {
            ...
            if (Root.Snapping == 2)
            {
                ...
                Root.SnappingRect = new Rectangle(left + Left, top + Top, width, height);
        ...
        public void ToTransparent()
        {
            ...
            WinApi.SetLayeredWindowAttributes(Handle, 0x00FFFFFF, 1, 0x2);
        }
        ...
        {
            uint dwExStyle = WinApi.GetWindowLong(Handle, -20);
            WinApi.SetWindowLong(Handle, -20, (uint)(dwExStyle & ~0x0020));
        }
        ...
        private bool[] LastPenStatus = new bool[10];
        private bool LastEraserStatus = false;
        private bool LastVisibleStatus = false;
        private bool LastPointerStatus = false;
        private bool LastPanStatus = false;
        private bool LastUndoStatus = false;
        private bool LastRedoStatus = false;
        private bool LastSnapStatus = false;
        private bool LastClearStatus = false;
        ...
        private void SetPenTipCursor()
        {
            ...
            if (dia <= 5)
            {
                Pen cpen = new Pen(Color.FromArgb(50, 128, 128, 128), 1.5f);
                dia += 6;
                g.DrawEllipse(cpen, (width - dia) / 2, (height - dia) / 2, dia, dia);
            }
            IC.Cursor = new System.Windows.Forms.Cursor(bitmaptip.GetHicon());
        }

        private short LastESCStatus = 0;
        ...
        private void tiSlide_Tick(object sender, EventArgs e)
        {
            ...
            {
                bool control = ((short)(WinApi.GetKeyState(VK_LCONTROL) | WinApi.GetKeyState(VK_RCONTROL)) & 0x8000) == 0x8000;
                bool alt = ((short)(WinApi.GetKeyState(VK_LMENU) | WinApi.GetKeyState(VK_RMENU)) & 0x8000) == 0x8000;
                bool shift = ((short)(WinApi.GetKeyState(VK_LSHIFT) | WinApi.GetKeyState(VK_RSHIFT)) & 0x8000) == 0x8000;
                bool win = ((short)(WinApi.GetKeyState(VK_LWIN) | WinApi.GetKeyState(VK_RWIN)) & 0x8000) == 0x8000;

                for (int p = 0; p < Root.MaxPenCount; p++)
                {
                    pressed = (WinApi.GetKeyState(Root.Hotkey_Pens[p].Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Eraser.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_InkVisible.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Undo.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Redo.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Pointer.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Pan.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Clear.Key) & 0x8000) == 0x8000;
                ...
                pressed = (WinApi.GetKeyState(Root.Hotkey_Snap.Key) & 0x8000) == 0x8000;
        ...
        private int IsMovingToolbar = 0;
        private Point HitMovingToolbareXY = new Point();
        private bool ToolbarMoved = false;
        ...
        private void FormCollection_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            short retVal = WinApi.GetKeyState(0x73);
